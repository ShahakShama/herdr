...
//!   * **head drift** — the worktree HEAD no longer matches `origin/<head>`
...
pub struct PrReviewDrift {
    ...
    /// The worktree HEAD differs from `origin/<head>` in either direction (the
    /// review no longer reflects the PR target). Implied by `head_behind > 0`,
    /// but also true when the worktree is *ahead* (e.g. local commits).
...
/// Pure drift decision, split out so it can be unit-tested without git. A `None`
/// remote oid (fetch/rev-parse failed) is treated as "can't tell" → no drift,
/// so a transient git error never raises a false badge.
fn compute_drift(
    ...
    let head_drifted = matches!((local_head, remote_head), (Some(a), Some(b)) if a != b);
    let base_moved = matches!((review_base_oid, remote_base), (Some(a), Some(b)) if a != b);
    PrReviewDrift {
        head_behind: if head_drifted { head_behind } else { 0 },
        head_drifted,
        base_moved,
    }
}
...
mod tests {
    ...
    fn clean_when_head_and_base_match() {
        let drift = compute_drift(Some("aaa"), Some("aaa"), 0, Some("bbb"), Some("bbb"));
    ...
    fn head_drift_carries_behind_count() {
        let drift = compute_drift(Some("old"), Some("new"), 3, Some("bbb"), Some("bbb"));
    ...
    fn base_moved_when_origin_base_advances_past_spawn_oid() {
        let drift = compute_drift(Some("aaa"), Some("aaa"), 0, Some("base_old"), Some("base_new"));
        assert!(drift.base_moved);
        assert!(!drift.head_drifted);
        assert!(!drift.is_clean());
    }
    ...
    #[test]
    fn unknown_remote_oid_is_not_drift() {
        // A failed fetch/rev-parse (None remote) must never raise a false badge.
        let drift = compute_drift(Some("aaa"), None, 0, Some("base_old"), None);
        assert!(drift.is_clean());
    }

    #[test]
    fn missing_spawn_base_oid_means_base_not_judged() {
        let drift = compute_drift(Some("aaa"), Some("aaa"), 0, None, Some("base_new"));
        assert!(!drift.base_moved);
    }
}

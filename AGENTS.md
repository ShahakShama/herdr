...
- **UI patterns should be reused.** Herdr is a keyboard-first TUI: the home surface has three panes — Control (repos, top-left), Agents (bottom-left), and Main (right) — and every action is a direct `alt`/`ctrl` chord (`alt+h/j/k/l` focus, `alt+n/r/x/p/s/,/?/q`, arrows for selection). The mouse only passes through to the focused Main pane; there is no mouse-driven chrome (tabs, sidebar drag, context menus). New dialogs, onboarding, settings, and post-update flows should follow the existing keyboard-first UI/UX language and interaction patterns instead of inventing one-off screens. Prefer reusing existing modal/screen structure, affordances, and close actions so the app feels consistent.
